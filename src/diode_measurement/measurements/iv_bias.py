...
def write_meta(writer: Writer, state: State) -> None:
    ...
    writer.write_tag(
        "bias_voltage[V]", safe_format(state.bias_voltage, writer.value_format)
    )
    writer.write_tag(
...
def write_iv_bias_reading(writer: Writer, reading: IVReading) -> None:
    ...
    row = (
        [
            ...
            safe_format(reading.v_smu2, writer.value_format),
            safe_format(reading.i_smu2, writer.value_format),
