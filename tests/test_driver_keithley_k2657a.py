...
def test_k2657a_adapter(res):
    ...
    assert res.buffer == ["smua.source.levelv = 4.200000E+01", "*OPC?"]

    res.buffer = ["1"]
    assert d.set_voltage_range(200.0) is None
    assert res.buffer == ["smua.source.rangev = 2.000000E+02", "*OPC?"]

    res.buffer = ["1"]
    assert d.set_current_compliance_level(0.002) is None
    assert res.buffer == ["smua.source.limiti = 2.000000E-03", "*OPC?"]
